window.templateExports = {
    ...
    render: (data) => {
        ...
        return `<div class="template-creative" style="font-family:'Trebuchet MS', sans-serif; max-width:950px; margin:0 auto; background:white; border-radius:24px; box-shadow:0 12px 24px rgba(0,0,0,0.05); overflow:hidden;">
            ...
            <div style="padding:2rem;">
                ...
                ${workHtml && `<div style="margin-bottom:1.5rem;"><h3 style="color:#2c5530; border-left:4px solid #d4af37; padding-left:0.8rem;">💼 Experiencia profesional</h3>${workHtml}</div>`}
                ${eduHtml && `<div style="margin-bottom:1.5rem;"><h3 style="color:#2c5530; border-left:4px solid #d4af37; padding-left:0.8rem;">🎓 Formación</h3>${eduHtml}</div>`}
                ${achievementsHtml && `<div style="margin-bottom:1.5rem;"><h3 style="color:#2c5530; border-left:4px solid #d4af37; padding-left:0.8rem;">🏆 Logros destacados</h3>${achievementsHtml}</div>`}
                ${projectsHtml && `<div style="margin-bottom:1.5rem;"><h3 style="color:#2c5530; border-left:4px solid #d4af37; padding-left:0.8rem;">💻 Proyectos personales</h3>${projectsHtml}</div>`}
                ${skillsHtml && `<div style="margin-bottom:1.5rem;"><h3 style="color:#2c5530; border-left:4px solid #d4af37; padding-left:0.8rem;">🛠️ Habilidades</h3>${skillsHtml}</div>`}
                ${languagesHtml && `<div style="margin-bottom:1.5rem;"><h3 style="color:#2c5530; border-left:4px solid #d4af37; padding-left:0.8rem;">🌐 Idiomas</h3>${languagesHtml}</div>`}
                ${certsHtml && `<div style="margin-bottom:1.5rem;"><h3 style="color:#2c5530; border-left:4px solid #d4af37; padding-left:0.8rem;">📜 Certificaciones</h3>${certsHtml}</div>`}
                ${refHtml && `<div><h3 style="color:#2c5530; border-left:4px solid #d4af37; padding-left:0.8rem;">📞 Referencias</h3>${refHtml}</div>`}
            </div>
        </div>`;
    }
};