...
// Configuración de listas dinámicas (igual que antes)
const listConfigs = {
    work: {
        ...
        fields: [
            ...
            { name: "startDate", label: "Desde", type: "text", placeholder: "Ene 2020" },
            { name: "endDate", label: "Hasta", type: "text", placeholder: "Actualidad" },
            { name: "description", label: "Descripción", type: "textarea", placeholder: "Logros, responsabilidades...", fullWidth: true }
        ],
        gridCols: "1fr 1fr"
    },
    edu: {
        ...
        fields: [
            { name: "study", label: "Estudio", type: "text", placeholder: "Ingeniería, Bootcamp..." },
            { name: "institution", label: "Institución", type: "text", placeholder: "Universidad, plataforma" },
            ...
            { name: "duration", label: "Duración", type: "text", placeholder: "200 horas / 6 meses" },
    ...
    ref: {
        ...
        fields: [
            ...
            { name: "phone", label: "Teléfono", type: "text", placeholder: "+123456789" },
            { name: "relation", label: "Relación", type: "text", placeholder: "Ex jefe, compañero..." }
        ],
...
function renderSocialLinks(template, personal) {
    const socials = [];
    ...
    if (template === 'minimal') {
        return `<div><strong>Redes sociales:</strong> ${socials.map(s => `${s.name}: ${escapeHtml(s.value)}`).join(' | ')}</div>`;
    } else {
        return `<div>${socials.map(s => `<strong>${s.emoji} ${s.name}:</strong> ${escapeHtml(s.value)}`).join(' | ')}</div>`;
    }
}

function renderWorkExperiences(workExperiences) {
    const items = workExperiences.filter(w => w.jobTitle || w.company);
    if (!items.length) return '<p>Sin experiencia registrada</p>';
    return items.map(w => `<div><strong>${escapeHtml(w.jobTitle)}</strong> en ${escapeHtml(w.company)} (${escapeHtml(w.startDate)} - ${escapeHtml(w.endDate)})<br><span>${escapeHtml(w.description)}</span></div><br>`).join('');
}
...
function renderEducation(educationList) {
    ...
    return items.map(e => `<div><strong>${escapeHtml(e.study)}</strong> - ${escapeHtml(e.institution)} (${escapeHtml(e.periodStart)}/${escapeHtml(e.periodEnd)}) - ${escapeHtml(e.status)} | Duración: ${escapeHtml(e.duration)}<br><span>${escapeHtml(e.description)}</span></div><br>`).join('');
}

function renderReferences(template, references) {
    const items = references.filter(r => r.name);
    if (!items.length) return '<p>No hay referencias</p>';
    if (template === 'minimal') {
        return items.map(r => `<div>${escapeHtml(r.name)} - ${escapeHtml(r.relation)} | Teléfono: ${escapeHtml(r.phone)}</div>`).join('');
    } else {
        return items.map(r => `<div>${escapeHtml(r.name)} - ${escapeHtml(r.relation)} | 📞 ${escapeHtml(r.phone)}</div>`).join('');
    }
}
    ...
    array.forEach((item, idx) => {
        ...
        for (let f of cfg.fields) {
            ...
            if (f.type === 'select') {
                ...
                fieldsHtml += `<div style="grid-column:1/-1;"><label>${f.label}</label><textarea data-field="${f.name}" data-idx="${idx}" rows="2" placeholder="${f.placeholder || ''}">${escapeHtml(value)}</textarea></div>`;
            } else {
                fieldsHtml += `<div><label>${f.label}</label><input type="${f.type}" data-field="${f.name}" data-idx="${idx}" value="${escapeHtml(value)}" placeholder="${f.placeholder || ''}"></div>`;
            }
        }
        fieldsHtml += `</div>`;
...
function renderPersonalStep() {
    ...
    const html = `
        ...
        <div class="form-grid">
            ${['fullName','birthDate','address','mobilePhone','homePhone','email','facebook','instagram','github'].map(f => {
                ...
                return `<div class="form-group"><label>${label}</label><input type="${type}" id="${f}" value="${escapeHtml(p[f])}"></div>`;
            }).join('')}
            <div class="form-group" style="grid-column:1/-1;"><label>Resumen profesional / Perfil</label><textarea id="profileSummary" rows="3" placeholder="Breve descripción de tu perfil profesional...">${escapeHtml(p.profileSummary)}</textarea></div>
    ...
    personalFields.forEach(f => {
        document.getElementById(f)?.addEventListener('input', (e) => { cvData.personal[f] = e.target.value; updatePhotoPreview(); });
    });
...
// ========== PASO FINAL: SELECTOR DE PLANTILLAS (sin fetch) ==========
function renderPreviewAndTemplateStep() {
    // Usar window.cvTemplatesList que viene de config.js
    ...
    document.querySelectorAll('.template-option').forEach(opt => {
        opt.addEventListener('click', async (e) => {
            ...
            try {
                ...
                renderPreviewAndTemplateStep(); // refrescar selector y preview
            } catch (err) {
                console.error(err);
